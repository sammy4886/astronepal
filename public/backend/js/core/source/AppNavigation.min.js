(function (namespace, $) {
	...
	p._enableEvents = function () {
		...
		$('#main-accounthead').on('click', 'li', function (e) {
			o._handleMenuItemClick(e);
		});
		$('#main-accounthead').on('click', 'a', function (e) {
	...
	p._handleMenubarEnter = function (menuItem) {
		...
		// Check if the accounthead should open
		if ((menuItemClicked === true || offcanvasVisible === false) && menubarExpanded !== true) {
			...
			if (menuItemClicked === false) {
				...
				if (this._lastOpened) {
					...
					var item = $('#main-accounthead > li.active');
	...
	p._handleMenubarLeave = function () {
		...
		// Never close the accounthead on extra small viewports
		if (materialadmin.App.isBreakpoint('xs') === false) {
			this._closeSubMenu($('#main-accounthead'));
		}
	};
	...
	p._openSubMenu = function (item, duration) {
		...
		item.find('> ul').stop().slideDown(duration, function () {
			...
			$('#main-accounthead ul').removeAttr('style');
		});
	};
	...
	p._invalidateMenu = function () {
		...
		var selectedLink = $('#main-accounthead a.active');

		// Expand all parent submenu's of the active link so it will be visible on startup
		selectedLink.parentsUntil($('#main-accounthead')).each(function () {
			if ($(this).is('li')) {
		...
		// When invalidating, dont expand the first submenu when the accounthead is collapsed
		if (this.getMenuState() === AppNavigation.MENU_COLLAPSED) {
			$('#main-accounthead').find('> li').removeClass('expanded');
		}

		// Check if the accounthead is visible
		if ($('body').hasClass('menubar-visible')) {
			this._handleMenubarEnter();
		}

		// Trigger event
		$('#main-accounthead').triggerHandler('ready');
	...
	p._evalMenuScrollbar = function () {
		...
		// Check if there is a accounthead
		var menu = $('#menubar');
		if (menu.length === 0)
			return;
